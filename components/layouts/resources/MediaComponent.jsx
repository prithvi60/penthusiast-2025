    ...
    return (
        <section className='padding w-full space-y-14'>
            ...
            <div className='w-full grid grid-cols-1 place-content-center place-items-center sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-y-10'>
                <Gradient rotate />
                {media.map((list, idx) => (
                    <Link title="media post page" key={idx} href={list.link} className='hover:scale-110 transition-all duration-500 ease-in-out'>
                        <Image title='article picture' src={list.img} alt='article image' width={380} height={450} className='object-cover object-center shadow-lg rounded-lg' />
                    </Link>
                ))}
            </div>
        </section>
